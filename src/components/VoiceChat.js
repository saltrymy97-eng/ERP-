...
function VoiceChat({ onClose }) {
  ...
  const startListening = () => {
    ...
    if (!SpeechRecognition) {
      setStatus('❌ عذراً، المتصفح الحالي لا يدعم بروتوكولات المعالجة الصوتية النطقية');
    ...
    recognition.onerror = (event) => {
      setStatus('❌ تداخل شبكي أو تم رفض تصريح الميكروفون السيادي');
      setListening(false);
      setCurrentStage('idle');
    };
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(214,175,55,0.1)', paddingBottom: '15px' }}>
          <div>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.82rem', margin: '4px 0 0 0' }}>تقنية معالجة اللغات الطبيعية والبث البيومتري المباشر للبيانات</p>
          </div>
            ...
            {currentStage !== 'idle' && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                {[...Array(12)].map((_, i) => {
                  ...
                  let color = 'var(--gold-main)';
                  if (currentStage === 'thinking') { color = 'var(--emerald-light)'; duration = 0.3; }
                  if (currentStage === 'speaking') { color = 'var(--green-bright)'; duration = 0.6; }
        ...
        <div className="voice-conversation-container" style={{ height: '220px', overflowY: 'auto', padding: '10px 5px', display: 'flex', flexDirection: 'column', gap: '15px', scrollbarWidth: 'thin' }}>
          <AnimatePresence>
            {conversation.map((msg, i) => (
              ...
              >
                <span style={{ fontSize: '0.75rem', fontWeight: 700, color: msg.role === 'user' ? 'var(--gold-main)' : 'var(--emerald-light)', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '4px' }}>
          ...
          {conversation.length === 0 && (
            <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-secondary)', display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'center' }}>
        ...
        <AnimatePresence>
          {status && (
            <motion.p 
              ...
              style={{ margin: 0, fontSize: '0.85rem', color: currentStage === 'listening' ? 'var(--gold-main)' : currentStage === 'thinking' ? 'var(--emerald-light)' : 'var(--green-bright)', textAlign: 'center', fontWeight: 600, background: 'rgba(0,0,0,0.15)', padding: '8px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.02)' }}
            >
              {status}
            </motion.p>
          )}
        ...
        <div className="voice-actions-lux" style={{ display: 'flex', gap: '12px', marginTop: '5px' }}>
          <motion.button
            ...
            style={{
              ...
              background: listening ? 'linear-gradient(135deg, #ef4444, #b91c1c)' : 'linear-gradient(135deg, var(--gold-main), #b89324)',
          ...
          <motion.button 
            ...
            style={{ flex: 1, background: 'rgba(255,255,255,0.02)', border: '1px solid var(--glass-border)', color: 'var(--text-secondary)', borderRadius: '14px', fontWeight: 700, cursor: 'pointer' }}
          >
            إغلاق النافذة
          </motion.button>
        </div>
