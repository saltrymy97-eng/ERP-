// src/components/Students.js – إدارة شؤون الطلاب والكليات (SQLite محلية حقيقية)
...
function Students() {
  ...
  const loadDepartments = useCallback(async (collegeId = null) => {
    if (collegeId) {
      ...
      const data = await getQuery(
        "SELECT d.*, c.name as college_name FROM departments d LEFT JOIN colleges c ON d.college_id = c.id WHERE d.status = 'active' ORDER BY d.name"
      );
      setDepartments(data || []);
    }
  }, []);
  ...
  const loadMajors = useCallback(async (deptId = null) => {
    if (deptId) {
      const data = await getQuery(
        "SELECT * FROM majors WHERE department_id = ? AND status = 'active' ORDER BY name",
        [deptId]
      );
      setMajors(data || []);
    } else {
      const data = await getQuery(
        "SELECT m.*, d.name as department_name, c.name as college_name FROM majors m LEFT JOIN departments d ON m.department_id = d.id LEFT JOIN colleges c ON d.college_id = c.id WHERE m.status = 'active' ORDER BY m.name"
      );
      setMajors(data || []);
    }
  }, []);
  ...
  const loadStudents = useCallback(async () => {
    ...
    const attendanceData = await getQuery(
      `SELECT student_id, status FROM attendance WHERE student_id IN (${placeholders})`,
      studentIds
    );
  ...
  const handleSave = async () => {
    ...
    if (tab === 'colleges') {
      if (editId) {
        await runQuery("UPDATE colleges SET name = ? WHERE id = ?", [formData.name, editId]);
      } else {
        await runQuery("INSERT INTO colleges (name, status) VALUES (?, 'active')", [formData.name]);
      }
      await loadColleges();
    }
    ...
    if (tab === 'departments') {
      if (editId) {
        await runQuery("UPDATE departments SET name = ?, college_id = ? WHERE id = ?", [formData.name, formData.college_id, editId]);
      } else {
        await runQuery("INSERT INTO departments (name, college_id, status) VALUES (?, ?, 'active')", [formData.name, formData.college_id]);
      }
      await loadDepartments();
    }
    ...
    if (tab === 'majors') {
      if (editId) {
        await runQuery("UPDATE majors SET name = ?, department_id = ?, fees = ?, duration = ? WHERE id = ?", 
          [formData.name, formData.department_id, formData.fees, formData.duration, editId]);
      } else {
        await runQuery("INSERT INTO majors (name, department_id, fees, duration, status) VALUES (?, ?, ?, ?, 'active')", 
          [formData.name, formData.department_id, formData.fees, formData.duration]);
      }
      await loadMajors();
    }
    ...
    if (tab === 'students') {
      if (editId) {
        ...
        await runQuery(
          "INSERT INTO students (university_id, full_name, phone, parent_phone, national_id, major_id, level, group_name, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active')",
