...
async function callGroq(messages, maxTokens = 300, temperature = 0.4) {
  ...
  try {
    ...
    return '⚠️ تعذر الاتصال بالمستشار الأكاديمي. تأكد من الاتصال بالإنترنت وصحة مفتاح الـ API.';
  }
}
...
export async function askAI(question, context = '', options = {}) {
  ...
  if (!isLoaded) {
    ...
    if (!loaded) return '⚠️ تعذر الاتصال بالمستشار الأكاديمي. يرجى التحقق من صحة المفتاح في صفحة الإعدادات.';
  }
...
// ==========================================
// ٤. تحليلات جاهزة (باستخدام SQLite)
// ==========================================
...
export async function analyzeDailyAttendance() {
  ...
  const schedules = await getQuery("SELECT * FROM schedules") || [];
...
export async function comprehensiveAnalysis() {
  const students = await getQuery("SELECT * FROM students WHERE status = 'active'") || [];
  const devices = await getQuery("SELECT * FROM devices") || [];
  const notifications = await getQuery("SELECT * FROM notifications") || [];
  const schedules = await getQuery("SELECT * FROM schedules") || [];

  const today = new Date().toISOString().slice(0, 10);
  const todayAttendance = await getQuery("SELECT * FROM attendance WHERE date = ?", [today]) || [];
